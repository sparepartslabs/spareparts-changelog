...
export async function updateChangelog(root, relative, a) { const file = path.resolve(root, relative); if (file !== root && !file.startsWith(root + path.sep))
    ...
    next = `# Changelog\n\n${a.markdown}\n`; if (next === old)
    return false; await fs.mkdir(path.dirname(file), { recursive: true }); const tmp = file + `.tmp-${process.pid}`; await fs.writeFile(tmp, next); await fs.rename(tmp, file); return true; }
