...
export async function run(request, deps = {}) { const root = deps.root ?? process.cwd(); validateRequest(request, root); const { entries, omitted } = categorize(await readHistory(request.from, request.to, root)); const artifact = render(request.identity, entries, omitted); const destinations = []; let repositoryChanged = false; if (request.writeRepository) {
