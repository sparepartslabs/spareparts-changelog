...
export function render(identity, entries, omittedCount = 0) { const key = createHash("sha256").update(identity).digest("hex").slice(0, 16); const lines = [`<!-- spareparts-changelog:start:${key} -->`, `## ${esc(identity)}`, ""]; for (const cat of order) {
    ...
    lines.push("_No user-visible changes._", ""); lines.push(`<!-- spareparts-changelog:end:${key} -->`); const markdown = lines.join("\n"); return { identity, entries, omittedCount, markdown, contentDigest: createHash("sha256").update(markdown).digest("hex") }; }
