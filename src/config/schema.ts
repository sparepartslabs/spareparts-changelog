...
export function validateRequest(r: ReleaseRequest, root = process.cwd()) {
  if (!r.from || !r.to || !r.identity.trim())
    throw new Error("from, to, and title are required");
  if (!r.ai?.provider?.trim()) throw new Error("AI provider is required");
  const resolved = path.resolve(root, r.changelogPath);
