...
import { projectLinkedIn } from "./linkedin-content.js";
...
export class LinkedInPublisher implements Publisher {
  ...
  async publish(a: ReleaseArtifact): Promise<DestinationResult> {
    try {
      ...
      if (!res.ok) throw new Error(`LinkedIn rejected request (${res.status})`);
      return {
