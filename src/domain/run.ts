...
import type { RunResult, DestinationResult } from "./result.js";
import { validateRequest } from "../config/schema.js";
import { readHistory } from "../git/history.js";
import { generateChanges, type AIFetcher } from "../ai/generator.js";
import { render } from "../render/markdown.js";
import { updateChangelog } from "../changelog/update.js";
import { S3Publisher, type S3Sender } from "../publishers/s3.js";
import { LinkedInPublisher, type Fetcher } from "../publishers/linkedin.js";
...
export async function run(
  ...
  const { entries, omitted } = await generateChanges(
    await readHistory(request.from, request.to, root),
    request.ai,
    deps.aiFetch,
  );
  ...
  if (request.writeRepository) {
    repositoryChanged = await updateChangelog(
      root,
      request.changelogPath,
      artifact,
    );
    destinations.push({
  ...
  if (request.s3.enabled)
    destinations.push(
      await new S3Publisher(request.s3, deps.s3Client).publish(artifact),
    );
  else
    destinations.push({
      destination: "s3",
  ...
  if (request.linkedin.enabled)
    destinations.push(
      await new LinkedInPublisher(request.linkedin, deps.fetch).publish(
        artifact,
      ),
    );
  else
